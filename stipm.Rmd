...
Saved output from the OAT runs is read back into memory and joined with observations to allow examination of performance skill.
...
Deterministic trailing means - Not sure we want/need this?

```{r trailing_means}
#create a temp col of the best case values that would be available in a given forecast year (i.e., up to year - 1)
#use slider to create rolling mean over 3 and 5 prior values
#this probably provides more recent return data than would truly be available...
rtrn_mu <- coho_data_tbl |> 
  select(StockID, pop_id, pop, year, rtrn) |>
  group_by(StockID, pop_id, pop) |> 
  mutate(
    rtrn_avail = c(NA, head(rtrn, -1)),
    rtrn_mu_3 = slider::slide_dbl(rtrn_avail, mean, .before = 2),
    rtrn_mu_5 = slider::slide_dbl(rtrn_avail, mean, .before = 4)
  ) |> 
  ungroup() |> 
  filter(year >= 2009) |> select(-rtrn_avail) |>  
  pivot_longer(cols = contains("rtrn_mu"), names_to = "lag_mod", values_to = "50%") |> 
  mutate(
    err = `50%` - rtrn, 
    err_abs = abs(err),
    err_log = log(`50%`) - log(rtrn),
...
#no diffs between lags for Rhat
oat |> 
  filter(StockID == 161) |> 
...
```{r starting_to_compare_trailing_mean}
oat_obs |> filter(StockID == 161) |> filter(str_detect(lag_mod, "stipm"))  |> print(n=100)
rtrn_mu |> filter(StockID == 161) |> arrange(lag_mod, year) |> print(n=100)
...
bind_rows(
  ...
  rtrn_mu |> 
    group_by(StockID, pop_id, pop, lag_mod) |> 
    summarise(across(err:err_abs_pct, list(mean = ~mean(.), median = ~median(.))), .groups = "drop") |> 
    select(StockID:lag_mod, contains("median"), contains("mean")) |> 
    filter(StockID == 161) 
) |> 
  gt::gt()
...
```{r first_look_other_pops}
pops <- c(161, 149, 145, 153, 23, 35)

oat_obs |> 
  filter(StockID %in% pops, lag_mod == "l2_stipm") |> 
  ggplot(aes(rtrn, `50%`, label = year)) + 
  geom_abline(slope = 1, intercept = 0) +
  geom_point() + 
  geom_pointrange(aes(ymin = `25%`, ymax = `75%`)) +
  ...
  scale_y_continuous("One-ahead forecast", labels = scales::comma) +
  facet_wrap(~StockID+pop, ncol = 2, scales = "free") +
  labs(subtitle = "l2_stipm, 50th + IQR on 1:1")

oat_obs |> 
  filter(StockID %in% pops, lag_mod == "l2_stipm") |> 
  ggplot(aes(year, group = StockID)) + 
  geom_col(data = filter(pre_oat, StockID %in% pops), aes(y = rtrn), fill = grey(0.8)) +
  geom_col(aes(y = rtrn)) +
  geom_line(aes(y = `50%`), color = 4, size = 1.2) + 
  facet_wrap(~StockID+pop, ncol = 2, scales = "free")
...
```{r oat_obs_pred_patchwork, eval=FALSE}
set_names(2009:2017) |> 
  map(function(x) {
...
#Grays Hbr Misc 16/157 missing 2000 & 2001 in FRAM data
stan_data_list$rtrn |> count(pop_id, pop, StockID) |> arrange(n) |> print(n=50)
coho_data_tbl |> filter(StockID == 157) |> print(n=50) #missing 2000 & 2001
coho_data_tbl |> count(pop_id, pop, StockID) |> arrange(n) |>  print(n=50)
coho_data_tbl |> filter(StockID == 81) |> print(n=50) #missing 1996
