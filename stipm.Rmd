...
oat_obs <- left_join(
  ...
  bind_rows(rtrn_trail_mean_fcst) |> #count(lag_mod)
  mutate(
...
The raw prediction error (forecast - observed) is shown for the state space models alongside the deterministic mean and the recorded FRAM forecast.
...
animate(
  set_names(2009:2019) |> 
    ...
    ggplot(aes(x = year, group = year_pred)) +
    #observed in forecast year
    geom_point(aes(y = rtrn), color = 1, shape = 17, size = 1.8)  +
    #forecasts
    geom_pointrange(aes(y = `50%`, ymin = `2.5%`, ymax = `97.5%`, color = lag_mod), fatten = 1.02, position = position_dodge(width = 0.5), show.legend = T) +
    geom_point(aes(y = `50%`, color = lag_mod), size = 1.2, position = position_dodge(width = 0.5), show.legend = T) +
    #pre-forecast data
    geom_vline(aes(xintercept = year_data), linetype = "dotted", size = 1) +
    geom_line(aes(y = obs), color = grey(0.8), size = 1) +
    geom_point(aes(y = obs), color = grey(0.4), size = 1) +
    geom_point(aes(y = obs, color = lag_mod), size = 0.9) +
    
    scale_x_continuous(name = "", limits = c(1998, 2019), breaks = 1998:2019, labels = 1998:2019) +
    #scale_x_discrete(name = "", limits = as.character(1998:2019), drop = FALSE, guide = guide_axis(n.dodge = 2)) +
    scale_y_continuous(name = "Return", limits = c(0,200000), labels = scales::comma) + #limits drops point error when outside ymax
