...
Dataset updated by NK and TG, following RMIS release count fix.

```{r cwt_rec_rel, eval=FALSE}
...
cwt <- readxl::read_excel(
  ...
  filter(!is.na(pop_id)) |> 
  select(pop_id, pop, smolt_ocn_surv_pop, year, est_n_rec, est_n_rel)
...
write_csv(cwt, "data/rmis_cwts.csv")
...
#Note pop_id reindexing to 1:34 here
#could eventually do furthest upstream in popmeta when all datasets updated
...
#rather than original inversion of spwn via ER
coho_data_tbl <- pop_meta |> #only 34 rows but pop_id through 36
  left_join(read_csv(fp$data_fram), by = c("StockID", "StockLongName")) |> #filter(spwn < 0 | hvst < 0)
  mutate(spwn = if_else(spwn < 0, 0.01, spwn)) |> #filter(spwn < 0 | hvst < 0)
  left_join(
    tbl_coho |> 
      select(
        pop_id, year, smolt = `Smolt Abundance`
        #, est_n_rec = Fishery_Plus_Escapement, est_n_rel = Release_No
      ),
    by = c("pop_id", "year")
  ) |> 
  mutate(
  ...
  #updated CWTs already reindexed to 1:34
  left_join(read_csv(fp$data_cwt), by = c("pop_id", "pop", "smolt_ocn_surv_pop", "year")) |>
  arrange(pop, year)
...
#      MS_dat_N = stan_data$MS$Release_No,
      MS_dat_x = stan_data$MS$est_n_rec |> round() |> as.integer(), #stan expects integer...
      MS_dat_N = stan_data$MS$est_n_rel |> round() |> as.integer(),
...
fit_ar1 <- stan_ar1(stan_data = stan_data_list, n_iter = iter, n_chain = chain)
#can just drop the 'probs = p' arg to get 2.5% to 97.5% posterior intervals
#divergent transitions:
get_sampler_params(fit_ar1, inc_warmup = FALSE) |> 
...
get_sampler_params(fit_stipm, inc_warmup = FALSE) |> 
  map_dbl(~.x[, "divergent__"] |> sum()) |> sum()

# fit_stipm_pred <- 
summary(fit_stipm, pars = "adult_pred")$summary |>
  as.data.frame() |> rownames_to_column("stan_out") |> tibble() |>
  mutate(
    var = str_extract(stan_out, "[a-z]+[:punct:][a-z]+"),
    pop_id = as.numeric(str_extract(stan_out, "[0-9]+")),
