...
stan_data_list$y_min_max <- 1998:2040
...
# #cols for n_year, n_pop
# fit_ar1 |> 
#   gather_draws(adult_est[n_year,n_pop])

x <- spread_draws(fit_ar1, adult_est[n_year,n_pop] | n_pop) 

x |> ungroup() |> 
  select(n_year, .iteration, est = `34`) |> 
  #count(n_year) # n = chains * post-warmup iterations
  mutate(year = 1997+n_year) |> 
#  filter(.iteration < 5) |> 
  ggplot(aes(year, est)) + 
  #stat_slab() +
  stat_eye(point_size = 0.7, slab_fill = pal_lag_mod[1]) +
  # #cool/pretty but take awhile
  # stat_slab(aes(slab_alpha = stat(f))) +
