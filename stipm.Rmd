...
oat_year_min <- 1998
oat_years_max <- 2012:2017 #predicting 2013:2018

oat <- set_names(oat_years_max) |> 
  map(function(x) {
    print(x)
    #a year_max of 2014, predicting 2015, with default lags gives FRAM/spwn through 2012
    stan_data_list <- stan_data_filter(coho_data_tbl, year_min = oat_year_min, year_max = x) 
    ...
    # #but year_max 2014, pred 2015 with "compiling" gives FRAM/spwn through 2013
    ...
    return(bind_rows(fit_ar1_pred, fit_stipm_pred))

  })

saveRDS(oat, "test_oat.rds")
#about 10m/AR1 @2Kx4
#about 1h/STIPM @2Kx4
```
...
```{r oat_checks}
# oat_obs <- bind_rows(
#   bind_rows(oat) |> select(mod, year, StockID, pop_id, pop, `10%`:`90%`) |> pivot_longer(`10%`:`90%`, names_to = "pctl", values_to = "val")
#   ,
#   coho_data_tbl |> mutate(mod = "obs", pctl = "obs", val = rtrn) |>  select(mod, year, StockID, pop_id, pop, pctl, val)
#   )

oat <- readRDS("test_oat.rds")
...
oat_obs <- left_join(
  ...
  select(mod, year, pop_id, pop, scale_err, rtrn, `10%`:`90%`) |>
  mutate(
...
oat_obs |> filter(pop_id == 34) |> filter(mod == "stipm")
