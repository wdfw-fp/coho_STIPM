...
Pre-CoTC 2020 escapement estimates that feed postseason BackwardsFRAM table. Not identical to forward FRAM resulting Escapement table values used in other years, but 1) should be close and 2) are what is available.

```{r}
# #unused
# "T:/DFW-Salmon Mgmt Modeling Team - General/Escapement files/Coho/fram_coho_escapement_2022.xlsx" |> 
#   readxl::read_excel(range = "BK2020!A6:D160")
...
predict(
  lm(log(hvst) ~ log(spwn), data = filter(coho_data_tbl, StockID==161, year>=2016)),
...
fit_stipm <- stan_stipm(stan_data = stan_data_list, n_iter = 400, n_chain = 2)
fit_ar1 <- stan_ar1(stan_data = stan_data_list, n_iter = 400, n_chain = 2)
...
#no 2022 when y_min_man ends 2021
fit_stipm_wb |> group_by(year) |> summarise(across(spwn:rtrn, ~quantile(.x, 0.5)), .groups = "drop") |> filter(year > 2018) 
#but can get distribution of draws on 2022 from adult_pred
#should bind_rows on fit_stipm_wb rather than force extra years in y_min_man
spread_draws(fit_stipm, adult_pred[n_pop] | n_pop) |> ungroup() |> 
  ...
  summarise(across(rtrn:year, ~quantile(.x, 0.5)), .groups = "drop") 
#matches the older summary()$summary shown below
fit_stipm_pred |> filter(StockID == 161)

fit_stipm_wb <- bind_rows(
  fit_stipm_wb,
  spread_draws(fit_stipm, adult_pred[n_pop] | n_pop) |> ungroup() |> 
    select(.chain, .iteration, .draw, rtrn = `34`) |> 
    mutate(year = last(stan_data_list$y_min_max)+1, mod = "stipm")
...
#get 2022 from AR1 adult_est...
fit_ar1_wb |> group_by(year) |> summarise(across(rtrn, ~quantile(.x, 0.5)), .groups = "drop") |> filter(year > 2018) 
