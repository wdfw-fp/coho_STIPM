...
fit_stipm <- stan_stipm(stan_data = stan_data_list, n_iter = 1000, n_chain = 2)
fit_ar1 <- stan_ar1(stan_data = stan_data_list, n_iter = 1000, n_chain = 2)
...
bind_rows(fit_ar1_pred, fit_stipm_pred) |> 
  filter(between(StockID, 130, 170)) |> 
  select(year, mod, StockID, pop, `2.5%`,`25%`,`50%`,`75%`,`97.5%`) |> 
  pivot_wider(names_from = mod, values_from = `2.5%`:`97.5%`) |> 
  select(year:pop, contains("50%"))
