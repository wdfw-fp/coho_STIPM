...
rows_upsert(
  ...
  left_join(
    ...
    by = c("StockID"))
  ,
  by = c("StockID", "year")) |> filter(year >= 2020) |> print(n=50)
...
summary(lm(hvst ~ spwn, data = filter(coho_data_tbl, StockID==161, year>1997)))
summary(lm(log(hvst) ~ log(spwn), data = filter(coho_data_tbl, StockID==161, year>1997)))
summary(lm(log(hvst) ~ log(spwn), data = filter(coho_data_tbl, StockID==161, year>2015)))



select(year, spwn, hvst, rtrn)
